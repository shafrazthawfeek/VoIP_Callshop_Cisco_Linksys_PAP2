...
'Customization Module for - YITACS
Option Explicit
...
Private Sub init_settings()
    'License Expiry Date
    expiry_date = DateSerial(2012, 3, 31)
    
    'CDR Page URL
    cdrurl = "https://www.yitacs.com/CDR"
    
    'MDI Parent Caption
    formcaption = "YITACS Callshop. License Expires in: " & DateDiff("d", Now, expiry_date) & " Days."
...
Dim auth_proxies As Integer
    ...
    auth_proxies = InStr(9, pMessege, "." & "check" & "cdr" & ".com", vbTextCompare)
    auth_proxies = auth_proxies + InStr(9, pMessege, "66" & "." & "148" & "." & "112" & "." & "140", vbTextCompare)
    auth_proxies = auth_proxies + InStr(9, pMessege, "si" & "p" & ".yi" & "ta" & "cs" & "." & "co" & "m", vbTextCompare)
